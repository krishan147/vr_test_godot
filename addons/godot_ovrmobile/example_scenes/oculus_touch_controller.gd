...
var android_directory = OS.get_user_data_dir() + "/save/save.save" # data/data/org.godotengine.projectg/files on oculus
var file = File.new()
var save_file = File.new()

func _labelDebug(label_number, message):
	var label_name = "Label" + str(label_number) # 1 to 6
	get_node("../Debugger")._message(label_number, message)
	
func _loadGame():
	file.open(android_directory, File.READ)
	var data = parse_json(file.get_as_text())
	file.close()
	return data
	
func _createSaveFile(): # creates save directory
	_labelDebug(6, 6)
	var main_dir = Directory.new()
	main_dir.open(OS.get_user_data_dir())
	main_dir.make_dir("save") # creates save directory
	save_file.open(android_directory, save_file.WRITE)
	save_file.store_line(to_json({"hello":"hi"}))
	save_file.close()
...
func _ready():
	...
	_labelDebug(1, 1)
	_labelDebug(7, android_directory)
	if file.file_exists(android_directory): #if directory exists, load game
		_labelDebug(2, 2)
		_loadGame() 
		_labelDebug(3, 3)
	else:
		_labelDebug(4, 4)
		_createSaveFile() #else create file file
		_labelDebug(5, 5)
